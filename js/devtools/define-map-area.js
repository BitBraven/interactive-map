import { devToolStates } from './dev-tools.js';
import { CANVAS_MAP } from '../config.js';
import { currentWallpaperMatrix } from '../canvas/canvas.js';
import { disableMouseInputTrick } from '../canvas/canvas-mouse-interactions.js';
import { getRelativeCoordinates } from '../utils.js';
import { createPopup, removePopup } from '../popup-manager.js'; //TEMP
...
const TOOL_PROMPTS = {
	'Define Map Tooltip Area': {
		...
		fields: [
			...
			{ label: 'Tooltip Description:', id: 'tooltipDesc', type: null, tag: 'textarea' }
		],
		buttons: [
			{ id: 'promptMapAreaTooltipCancel', text: 'Cancel', className: 'btn-cancel' },
	...
	'Define Region Sound Area': {
		...
		buttons: [
			{ id: 'promptMapSoundRegionCancel', text: 'Cancel', className: 'btn-cancel' },
...
function createPrompt({ id, fields, buttons }) {
...
function createPromptHandler(config) {
	return () => {
		return new Promise((resolve) => {
			const prompt = document.getElementById(config.promptId);
			const inputs = config.inputSelectors.map(sel => document.querySelector(sel));

			const cleanUp = () => {
				prompt.style.display = 'none';
				document.body.removeEventListener('keydown', handleKeyPress);
				config.buttonIds.forEach(id => {
					document.getElementById(id).removeEventListener('click', clickHandlers[id]);
				});
			};
...
async function saveAreasToFile() {
	disableMouseInputTrick[0] = false;
	const activeTool = devToolStates.find(tool => tool.state);
	if (!activeTool) return;
...
export function notifyMapAreaTool(toolName, toolState) {
